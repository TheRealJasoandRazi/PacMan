...
{
    public class Inky : GhostParent, Ghost
    {
        private int x;

        public Inky() : this(Color.Orange, 500, 200, 540, 200, 520, 240, "Inky")
        {
        }

        public Inky(Color clr, float x, float y, float x2, float y2, float x3, float y3, string n)
        {
            X2 = x2;
            X3 = x3;
            Y2 = y2;
        ...
        public override void travel()
        {
            ...
            public void Multiply(Characters c, GhostParent p)
            {
                if (SplashKit.TimerTicks("multiply") >= 10000)
                    {
                        Inky ink = new Inky();
                        c.CharacterAdd(ink);
                        p.GhostAdd(ink);
                        SplashKit.ResetTimer("multiply");
                    }
            }
    }
}
